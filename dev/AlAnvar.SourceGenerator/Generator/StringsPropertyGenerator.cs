    ...
    private void Execute(SourceProductionContext ctx, AdditionalText file)
    {
        ...
        lock (_generatedFileNames)
        {
            if (_generatedFileNames.Contains(fileName))
                ctx.ReportDiagnostic(Diagnostic.Create(Constants.FSG1003, Location.None, fileName));
        ...
        _ = sb.AppendLine();
        _ = sb.AppendLine($"namespace {Constants.HelperNamespace.Remove(Constants.HelperNamespace.Length - 1)}");
        _ = sb.AppendLine("{");
        ...
        _ = sb.AppendLine($"{tabString}/// </summary>");
        _ = sb.AppendLine($"{tabString}public sealed partial class {Constants.StringsClassName}");
